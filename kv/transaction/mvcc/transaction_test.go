...
func assertPutInTxn(t *testing.T, txn *MvccTxn, key []byte, value []byte, cf string) {
	// writes: all changes added to this transaction.
	writes := txn.Writes()
	assert.Equal(t, 1, len(writes))
	expected := storage.Put{Cf: cf, Key: key, Value: value}
	// writes就是事务对写请求做的记录
	// put：[{key value cfType}]
	// ok: 是否强转storage.Put类型成功（验证是否为put类型
...
func TestPutLock4A(t *testing.T) {
	// txn is MvccTxn with StartTS=42
	txn := testTxn(42, nil)
	lock := Lock{
		Primary: []byte{16},
		Ts:      100,
...
// UserKey为[]byte{16, 240}, TS为42的写入记录(write)
// UserKey为[]byte{16, 240}, TS为40的的KV数据(default)
...
func TestGetValueTooEarly4A(t *testing.T) {
	// 事务为41
	// 数据的提交时间为42，开始时间为40，所以这个事务不应该得到数据
